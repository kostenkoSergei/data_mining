...
def build_handshake_line(graph, users_list, target_users):
    """Build handshake line using deque from collection"""
    ...
    deq = deque([start])
    is_visited[start] = True  # flag

    while len(deq) > 0:
        current = deq.pop()
        if current == finish:
            break
        for i, top in enumerate(graph[current]):
            if top == 1 and not is_visited[i]:
                is_visited[i] = True
                parent[i] = current
                deq.appendleft(i)
    else:
...
def get_users_name_list(database: str, table: str):
    """returns distinct users names from db"""
    db = db_client[database]
    collection = db[table]
...
def create_graph_from_mongodb(database: str, table: str, users_list: list):
    """returns graph based on followers relationships"""
    graph = []
...
if __name__ == '__main__':
    ...
    print(build_handshake_line(graph, users_list, target_users))
